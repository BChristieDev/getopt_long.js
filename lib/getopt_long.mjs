 ...
 * @typedef Extern
 * @type {object}
 * @property {string|undefined} optarg
 * @property {number} optind
 * @property {number} opterr
 * @property {string|number} optopt
 */
 ...
 * @property {number} has_arg
 * @property {string[]|number[]|number|null} flag
 ...
 */
function typeCheckParams(argc, argv, shortopts, longopts, indexptr)
{
    ...
    if (Array.isArray(longopts))
    {
        longopts.forEach(longopt => {
            ...
            if (typeof longopt?.name !== 'string')
                throw new TypeError(`expected 'string' but element property 'name' is of type '${typeof longopt?.name}'`);
            ...
            if (typeof longopt?.flag[0] !== 'string' && typeof longopt?.flag[0] !== 'number' && typeof longopt?.flag !== 'number' && longopt?.flag !== null)
                throw new TypeError(`expected 'string[] | number[] | number | null' but element property 'flag' is of type '${longopt?.flag}'`);
    ...
    if (typeof indexptr[0] !== 'number')
        throw new TypeError(`expected 'number' but argument is of type '${typeof indexptr[0]}'`);
...
{
    const eq = argv[extern.optind].indexOf('=', 1);
    ...
    if (longopts[optarrIndex].has_arg === Constants.no_argument || eq === -1)
        extern.optind++;
...
{
    typeCheckParams(argc, argv, shortopts, longopts, indexptr);
