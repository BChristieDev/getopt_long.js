...
{
    // @ts-ignore
    return typeof Deno !== 'undefined' ? true : false;
}
...
function parseLongOpt(argc, argv, longopts, indexptr)
{
    ...
    const err = parseArg(argc, argv, isOptional, isRequired, 0, eq + 1, `${progname}: option '--${opt}' requires an argument`);

    if (err)
        return err;

    if (longopts[optarrIndex].flag === null || typeof longopts[optarrIndex].flag === 'number')
...
function parseShortOpt(argc, argv, shortopts)
{
    ...
    const err = parseArg(argc, argv, isOptional, isRequired, opt, nextchar, `${progname}: option requires an argument -- '${opt}'`);
